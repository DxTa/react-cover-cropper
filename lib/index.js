...
class Cropper extends React.Component {
  ...
  fitImageToCanvas (width, height) {
    ...
    var canvasAspectRatio = this.refs.canvas.parentNode.clientHeight / this.refs.canvas.parentNode.clientWidth;
    var imageAspectRatio = height / width;

    if (canvasAspectRatio > imageAspectRatio) {
      scaledHeight = this.refs.canvas.parentNode.clientHeight;
      let scaleRatio = scaledHeight / height;
      scaledWidth = width * scaleRatio;
    } else {
      scaledWidth = this.refs.canvas.parentNode.clientWidth;
  ...
  boundedCoords (x, y, dx, dy) {
    ...
    var rightEdge = this.refs.canvas.parentNode.clientWidth;
    var leftEdge = 0;
  ...
  componentDidUpdate () {
    ...
    context.clearRect(0, 0, this.refs.canvas.parentNode.clientWidth, this.refs.canvas.parentNode.clientHeight);
  ...
  addImageToCanvas (context, image) {
    ...
    x = Math.min(x, 0);
    y = Math.min(y, 0);
    y = scaledHeight + y >= this.refs.canvas.parentNode.clientHeight ? y : (y + (this.refs.canvas.parentNode.clientHeight - (scaledHeight + y)));
    x = scaledWidth + x >= this.refs.canvas.parentNode.clientWidth ? x : (x + (this.refs.canvas.parentNode.clientWidth - (scaledWidth + x)));
      ...
      <div className="AvatarCropper-canvas">
        <div style={{ position: "relative", width: this.props.width, height: this.props.height }}>
          ...
          <div style={{
            ...
            <div className='closeBtn' onClick={this.props.onRequestHide}>{this.props.closeButtonCopy}</div>
            <div className='saveBtn' onClick={this.handleCrop.bind(this)}>
...
CoverCropper.defaultProps = { width: '100%', height: '400px', modalSize: "large",
